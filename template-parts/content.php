    ...
    <div class="relative pt-16 before:absolute before:top-0 before:left-0 before:h-px before:w-6 before:bg-zinc-950 after:absolute after:top-0 after:right-0 after:left-8 after:h-px after:bg-zinc-950/10">
        <div class="relative lg:-mx-4 lg:flex lg:justify-end">
            ...
            <dl class="lg:absolute lg:top-0 lg:left-0 lg:w-1/3 lg:px-4">
                <dt class="sr-only"><?php _e('Published', 'webmakerr'); ?></dt>
                ...
                <dd class="mt-6 flex gap-x-4">
                    <div class="flex-none overflow-hidden rounded-xl bg-light">
                        <?php
                            echo get_avatar(get_the_author_meta( 'ID' ), 32, '', esc_attr(sprintf(__('Avatar for %s', 'webmakerr'), get_the_author())), [
            ...
            <a class="!no-underline mt-8 inline-flex rounded-full bg-zinc-950 px-4 py-1.5 text-sm font-semibold text-white transition hover:bg-zinc-800" aria-label="<?php echo esc_attr(sprintf(__('Read more: %s', 'webmakerr'), get_the_title())); ?>" href="<?php the_permalink(); ?>">
                <?php _e('Read more', 'webmakerr'); ?>
            </a>
        </div>
    </div>
</article>
